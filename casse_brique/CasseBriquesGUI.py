...
class CasseBriquesGUI(tk.Tk):
    ...
    def creer_widgets(self):
       ...
       """
        self.canvas = tk.Canvas(self, width=self.largeur_canevas, height=self.hauteur_canevas, bg='white')
        self.canvas.pack()
    ...
    def dessiner_brique(self, brique):
        ...
        brique.canvas_id = self.canvas.create_rectangle(x0, y0, x1, y1, fill=brique.couleur, outline='black')
    ...
    def animer_balle(self):
       ...
       # Si une brique est touchée
        ...
        self.canvas.itemconfig(self.score_text, text=f"Score : {self.casse_briques.score}")
